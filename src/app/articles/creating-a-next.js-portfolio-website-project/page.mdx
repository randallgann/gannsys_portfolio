...
Once Terraform is setup the development process will be the same as above with the exception that after the docker image is pushed to the appropriate GCP project container repo all that needs to be done is to deploy a new version is  to run the terraform commands.
